...
@pytest.mark.asyncio
async def test_legacy_router_profile_unknown_key_still_blocks_writes(tmp_path) -> None:
    """A legacy profile must not bypass external-change detection entirely."""
    target = tmp_path / "config.toml"
    config = GatewayConfig(config_path=str(target))
    payload = config.to_toml_dict()
    payload["provider_profiles"] = {
        "deepseek": {
            ...
            "agentos_router": {
                "enabled": True,
                "unrecognized_legacy_router_option": True,
            },
        }
    }
